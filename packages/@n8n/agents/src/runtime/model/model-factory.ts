 ...
 *
 * `@n8n/agents` is a standalone SDK and deliberately does not depend on
 * `@n8n/backend-network`, so it cannot build the backend's centrally-guarded
 * transport itself. Inside the n8n backend that guarded `fetch` is always
 * injected into {@link createModel} / {@link createEmbeddingModel} (see cli's
 * `createAiProxyFetch`, which wraps `@n8n/backend-network`), and this fallback
 * is never reached. It runs only when the SDK is used on its own with no
 * injected `fetch`, where it does the minimum to honor `HTTPS_PROXY` /
 * `HTTP_PROXY` for model calls.
 */
